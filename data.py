...
class SpeechDataset(Dataset):
    ...
    def __init__(self, root_dir: str, meta_info: str, transform: torch.nn.Sequential = None, speakers_count: int = 2,
                 sample_rate: int = 16000, duration: int = 4):
        ...
        self.speaker_dict = {us: e for e, us in enumerate(list(self.meta_info.client_id.unique()))}

    def __len__(self):
        return len(self.meta_info) / self.speakers_count

    def get_audio(self, audio_name: str) -> torch.Tensor:
        waveform, sample_rate = torchaudio.load(f'{self.root_dir}/clips/{audio_name}')
    ...
    def __getitem__(self, idx):
        ...
        speakers = torch.LongTensor([self.speaker_dict[s] for s in samples['client_id'].values])
        audios = torch.stack([self.crop_audio(self.get_audio(audio)) for audio in samples['path'].values])
